#!/usr/bin/env python
"""Execute migration 60_consolidate_forecast_tables_v10.sql"""
import pyodbc
...
def run_migration():
    ...
    try:
        ...
        migration_path = Path('scripts/sql/migrations/60_consolidate_forecast_tables_v10.sql')
        ...
        for i, batch in enumerate(batches, 1):
            try:
                ...
                conn.commit()
                # Extract batch description from comments
                first_line = batch.split('\n')[0]
                desc = first_line[2:].strip() if first_line.startswith('--') else f'Batch {i}'
                print(f'✓ [{i}/{len(batches)}] {desc}')
        ...
        cursor.close()
        conn.close()
        print('\n✓ Migration 60_consolidate completed successfully')
        return True
