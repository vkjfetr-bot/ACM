...
# acm_core.py
# Changelog
# 2025-10-06: Initial core utilities for ACM (no tag autodiscovery; contextual baselines; regimes; IF; drift; spans; SQL helpers).
...
def select_tags(df: pd.DataFrame, tags: List[str]) -> List[str]:
    # Strict: only use provided tags; ensure they exist and are numeric-coercible.
    missing = [t for t in tags if t not in df.columns]
    if missing:
        raise ValueError(f"Missing tags in data: {missing}")
    return tags
...
def _mad(x: pd.Series):
    ...
    mads = float(np.nanmedian(np.abs(x - m)))
...
def build_baseline(df_hist: pd.DataFrame, tags: List[str], kind="hour") -> Baseline:
    ...
    if kind == "hour":
        ...
        raise ValueError("baseline kind must be global|hour|weekday")
...
def health_index(score: pd.Series, spans: List[Tuple[pd.Timestamp, pd.Timestamp]]) -> float:
    # Simple, robust: combine score quantile + event density
    if score.empty: return 100.0
    q95 = float(score.quantile(0.95)) if score.notna().any() else 0.0
    density = len(spans) / max(1, len(score) / 60.0)  # spans per ~minute
    raw = 0.6 * q95 + 0.4 * density
    hi = 100.0 - 100.0 * (raw / (raw + 1.0))  # squashed 0..100
    return max(0.0, min(100.0, hi))

def health_label(hi: float, prev_label: Optional[str] = None) -> str:
    # Simple hysteresis if prev_label provided (optional)
...
# ---------- SQL helpers (pyodbc) ----------
def sql_connect(cnx_str: str):
    import pyodbc
    return pyodbc.connect(cnx_str)

def sql_exec(cur, sql: str, params: Tuple = ()):
    cur.execute(sql, params)

def upsert_artifact(cnx, equipment_id: str, kind: str, trained_from, trained_to, params_json: dict, active: bool, version: int = None):
    cur = cnx.cursor()
    if active and version is not None:
        sql_exec(cur, "UPDATE dbo.acm_artifact SET Active=0 WHERE EquipmentId=? AND ArtifactType=?", (equipment_id, kind))
    sql = """
    INSERT INTO dbo.acm_artifact (EquipmentId, ArtifactType, Version, TrainedFrom, TrainedTo, ParamsJSON, Active, CreatedAt)
    VALUES (?, ?, ?, ?, ?, ?, ?, SYSUTCDATETIME())
    """
    cur.execute(sql, (equipment_id, kind, version, trained_from, trained_to, json.dumps(params_json, ensure_ascii=False), 1 if active else 0))
    cur.commit()

def get_active_artifacts(cnx, equipment_id: str) -> Dict[str, dict]:
    cur = cnx.cursor()
    cur.execute("""
      SELECT ArtifactType, Version, ParamsJSON
      FROM dbo.acm_artifact
      WHERE EquipmentId=? AND Active=1
    """, (equipment_id,))
    rows = cur.fetchall()
    out = {}
    for r in rows:
        out[r.ArtifactType] = {"version": r.Version, "params": json.loads(r.ParamsJSON)}
    return out

def write_run_and_health(cnx, equipment_id: str, window_start, window_end,
                         rows_proc: int, tags_used: int, anomaly_frac: float, hi: float, hlabel: str,
                         drift_json: dict, baseline_ver: int, regime_ver: int) -> int:
    cur = cnx.cursor()
    cur.execute("""
      INSERT INTO dbo.acm_run_summary
        (EquipmentId, WindowStart, WindowEnd, RowsProcessed, TagsUsed, AnomalyFraction,
         HealthIndex, HealthLabel, DriftJSON, BaselineVersion, RegimeVersion, CreatedAt)
      OUTPUT INSERTED.RunId
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, SYSUTCDATETIME())
    """, (equipment_id, window_start, window_end, rows_proc, tags_used, anomaly_frac,
          hi, hlabel, json.dumps(drift_json, ensure_ascii=False), baseline_ver, regime_ver))
    run_id = cur.fetchone()[0]
    cur.commit()

    # Upsert asset health (common table)
    cur.execute("""
      MERGE dbo.acm_asset_health AS tgt
      USING (SELECT ? AS EquipmentId) AS src
      ON tgt.EquipmentId = src.EquipmentId
      WHEN MATCHED THEN UPDATE SET
            AsOfTime = ?, WindowStart=?, WindowEnd=?, HealthIndex=?, HealthLabel=?,
            AnomalyFraction=?, EventCount=ISNULL(EventCount,0), DominantState=NULL,
            DriftFlag=?, DataQualityScore=NULL, BaselineVersion=?, RegimeVersion=?, LastEventPeak=NULL, UpdatedAt=SYSUTCDATETIME()
      WHEN NOT MATCHED THEN
        INSERT (EquipmentId, AsOfTime, WindowStart, WindowEnd, HealthIndex, HealthLabel, AnomalyFraction, EventCount, DominantState,
                DriftFlag, DataQualityScore, BaselineVersion, RegimeVersion, LastEventPeak, UpdatedAt)
        VALUES (?, ?, ?, ?, ?, ?, ?, 0, NULL, ?, NULL, ?, ?, NULL, SYSUTCDATETIME());
    """, (equipment_id,
          window_end, window_start, window_end, hi, hlabel, anomaly_frac, 1 if drift_json else 0, baseline_ver, regime_ver,
          equipment_id, window_end, window_start, window_end, hi, hlabel, anomaly_frac, 1 if drift_json else 0, baseline_ver, regime_ver))
    cur.commit()
    return run_id

def write_events(cnx, run_id: int, equipment_id: str, spans: List[Tuple[pd.Timestamp, pd.Timestamp]],
                 score: pd.Series, states: pd.Series, top_tags: List[str]):
    if not spans: return
    cur = cnx.cursor()
    sql = """
      INSERT INTO dbo.acm_event
        (RunId, EquipmentId, StartTime, EndTime, PeakScore, ModeState, ContribTopTags, ContribNotes, Severity, Status)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """
    for (s, e) in spans:
        ...
        cur.execute(sql, (run_id, equipment_id, s.to_pydatetime(), e.to_pydatetime(),
                          peak, mode_state, ",".join(top_tags[:3]), None, severity, "Open"))
    cur.commit()
