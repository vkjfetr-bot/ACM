...
def fit_pca(df: pd.DataFrame, n_components: int = 5, out_dir: str | Path | None = None) -> PCAArtifacts:
    x = df.select_dtypes(include=[float, int]).astype(np.float32).values
    scaler = StandardScaler()
...
def score_h2(df: pd.DataFrame, artifacts: PCAArtifacts | None = None, art_dir: str | Path | None = None) -> pd.Series:
    if artifacts is None:
        if art_dir is None:
            raise ValueError("Provide artifacts or art_dir")
    ...
    med = float(np.median(err)); mad = float(np.median(np.abs(err - med)) + 1e-9)
    z = (err - med) / (1.4826 * mad)
    s = 1 / (1 + np.exp(-z))
    return pd.Series(s, index=df.index, name="H2_Recon")
